...
3. Defaults should already be: algorithm = Sobel, charset = acerola, 200 × 110, edgeSmoothness=2.0, edgeSensitivity=50, edgeDensity=50 (which internally maps to σ=2·cols/200, τ=1.0, k=1.6, threshold=0.005, tile=8). If not, the file is `app/templates.ts → DEFAULT_SETTINGS`.
...
| Slider (UI label / field) | Try | Expected |
| --- | --- | --- |
| `edgeSmoothness` ("Edge Smoothness") | 0.5 → 5 | Broader, smoother contours as it grows. Internally scaled by `cols / 200`. |
| `edgeSensitivity` ("Sensitivity") | 0 → 100 | Maps to DoG threshold via `0.005·10^(1−s/50)`. 0=0.05, 50=0.005, 100=0.0005. Higher = pick up fainter edges. |
| `edgeDensity` ("Edge Density") | 0 → 100 | Maps to per-tile vote min via `round(64·(1−d/100)³)`. 0=64 (no edges), 50=8 (default), 100=0 (every cell with any vote). |

Locked internally (not user-configurable): `kRatio = 1.6` (canonical Mexican-hat), `tau = 1.0` (Acerola default), `radius = clamp(ceil(2·effectiveSigma), 1, 10)`.
...
3. **Two separable Gaussian blurs** at σ and σ·k, kernel half-width auto-derived as `clamp(ceil(2·σ_effective), 1, 10)` (`gaussianBlurPair`). σ_effective = `edgeSmoothness · cols / 200` — resolution-adaptive so the same slider value means the same visual edge width regardless of output columns.
...
8. **Per-tile vote** over the 64 pixels of each 8×8 cell. If the dominant direction count ≥ tile threshold, emit that glyph in `EdgeData[col][row]`. The tile threshold is derived from the `edgeDensity` slider via `round(64·(1−d/100)³)`. Unlike Acerola, we explicitly skip the `-1` (no-direction) entries when tallying — Acerola's shader leaks those into `buckets[-1]` via undefined indexing.
...
- [app/components/output-options.tsx](app/components/output-options.tsx) — `predefinedCharacterSets` (incl. `acerola: ' .icoP0?@■'`). Character set selector moved out to preprocessing-options. Also houses the "Show Underlying Image" section with its sub-controls (separate color editing + the "Export pixelated image" PNG/SVG button pair).
- [app/components/asset-export.tsx](app/components/asset-export.tsx) — ASCII art export (PNG/SVG, frames, mp4, gif). `generateSvgContent` emits the text-layer SVG that's meant to overlay the pixelated SVG.
...
| `edgeSmoothness` | 2.0 | 0.1–5 | Acerola `_Sigma`. Scaled internally by `cols/200`. |
| `edgeSensitivity` | 50 | 0–100 | Acerola `_Threshold` via `0.005·10^(1−s/50)`. |
| `edgeDensity` | 50 | 0–100 | Acerola `_EdgeThreshold` via `round(64·(1−d/100)³)`. |

Locked internally (not user-configurable): `kRatio = 1.6` (Acerola `_SigmaScale`, canonical Mexican-hat), `tau = 1.0` (Acerola `_Tau`, balanced), kernel half-width = `clamp(ceil(2·σ_effective), 1, 10)`.
...
## Pixelated image export (PNG + SVG)

Two side-by-side buttons under an "Export pixelated image" label in the Underlying Image section. Both handlers live in [ascii-art-generator.tsx](app/components/ascii-art-generator.tsx) and share the same source-URL branch as the live preview's BG layer:

- `separateBgColorEditing === false` → read from `processedImageUrl` (color adjustments already baked in by the preprocessing pipeline).
- `separateBgColorEditing === true` → read from `rawProcessedImageUrl` and apply `ctx.filter` with the same CSS `brightness(...) contrast(...) invert(...)` formula the preview uses, so the export matches what's on screen.

**PNG** (`exportPixelatedImage`) draws the source onto a `cols·CHAR_WIDTH × rows·CHAR_HEIGHT` canvas with `imageSmoothingEnabled = false`, scaling non-uniformly to give cells the same aspect as the text renderer.

**SVG** (`exportPixelatedSVG`) reads pixels via `getImageData` at native `cols × rows`, then performs a two-pass greedy run-length merge: horizontal spans of identical RGBA per row, then vertical extension of open rects across rows when `x`/`width`/`color` all match. Each rect is emitted as `<rect ... fill="rgb(r,g,b)" fill-opacity="a/255"/>` (omit `fill-opacity` when `a===255`). Fully transparent pixels (`a===0`) break spans and are skipped. The `<svg>` carries `viewBox="0 0 cols rows"` with explicit non-square `width`/`height` and **`preserveAspectRatio="none"`** so 1-unit cells stretch to `CHAR_WIDTH × CHAR_HEIGHT`.

### Pixelated SVG ↔ ASCII SVG alignment contract

The pixelated SVG and the ASCII text SVG ([asset-export.tsx](app/components/asset-export.tsx) `generateSvgContent`) are designed to overlay pixel-perfectly when imported into a vector editor. To make that true:

- **Cell width is measured, not constant.** Both exports build a hidden 10-`X` `<span>` in `GT America Mono` at `fontSize=12px`, read `getBoundingClientRect().width / 10`, and use that as the per-cell horizontal advance. `CHAR_WIDTH = 7.45` is only an estimate of this value — the PNG export still uses it (raster, no overlay), but neither SVG export should.
- **Cell height is `CHAR_HEIGHT = 14.4`**, which equals `fontSize * 1.2` in the ASCII export by construction. Both stay in sync.
- **ASCII SVG draws no background and no padding.** `padding` is forced to `0` and the `<rect width="100%" height="100%" fill="${backgroundColor}"/>` background is omitted, so character `(0,0)` lands at SVG `(0,0)` and the pixelated layer beneath shows through. The `exportSettings.padding` and `exportSettings.backgroundColor` controls no longer affect the SVG output — only the PNG path.
...
- **Pixelated image export sizing: use CHAR_WIDTH × CHAR_HEIGHT, not a uniform scale.** Character cells are `7.45 × 14.4` px (`CHAR_WIDTH`/`CHAR_HEIGHT` in [dimension-utils.ts](app/components/dimension-utils.ts)) — not square. Scaling the `cols × rows` source image by a uniform factor (e.g. ×8) produces a horizontally stretched result. The PNG export canvas should be `Math.round(cols * CHAR_WIDTH) × Math.round(rows * CHAR_HEIGHT)`.
- **Pixelated SVG needs `preserveAspectRatio="none"`.** `viewBox` is `0 0 cols rows` (square units) but the explicit `width`/`height` are non-square (`cols·measuredCellWidth × rows·CHAR_HEIGHT`). With the default `xMidYMid meet`, renderers preserve the square viewBox ratio and letterbox — the SVG renders fine in some browsers and gets stretched/cropped in vector editors. `preserveAspectRatio="none"` lets the 1-unit rects stretch to the cell aspect, matching the PNG.
- **Pixelated SVG fill format: `rgb()` + `fill-opacity`, not `rgba()`.** Chromium renders `<rect fill="rgba(r,g,b,a)"/>` fine, but Illustrator / Affinity / Inkscape often parse it as black or transparent. Always emit SVG-1.1-compliant `fill="rgb(r,g,b)"` plus a separate `fill-opacity="a/255"` attribute (and omit `fill-opacity` entirely when `a===255` to keep the file lean).
- **Both SVG exports must measure cell width, not use `CHAR_WIDTH`.** The pixelated SVG and the ASCII SVG are designed to overlay in a vector editor. `CHAR_WIDTH = 7.45` is an *estimate* of GT America Mono's natural advance at 12px — the actual computed advance is the source of truth. Both exports build a hidden 10-`X` `<span>` and divide its measured width by 10. If you ever bypass that (e.g. switch one back to `CHAR_WIDTH`), the two layers drift apart by a sub-pixel-per-column that compounds across the row.
- **ASCII SVG export deliberately ignores `padding` and `backgroundColor`.** `padding` is forced to 0 and the background `<rect>` is omitted so the SVG overlays the pixelated SVG cleanly. The `exportSettings.padding` / `exportSettings.backgroundColor` controls in the UI still apply to PNG export and elsewhere — just not to the SVG output path. If a future request brings these back for SVG, gate them behind a new toggle rather than re-enabling them unconditionally.
